import BaseServer from "trhr/if.server";
...
export function numCycleForGrowthTransition(server, growth, p, cores = 1) {
  return numCycleForGrowthCorrected(
    server,
    server.moneyAvailable * growth,
...
export function numCycleForGrowthByHackAmt(
  ...
  const posthackMoney = Math.floor(
    prehackMoney * Math.min(1, Math.max(0, 1 - hackProp))
  );
  return numCycleForGrowthCorrected(
    server,
    prehackMoney,
