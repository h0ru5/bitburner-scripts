...
import { numCycleForGrowthCorrected } from "trhr/if.server.hackable";
...
export async function main(ns) {
  ...
  let weakThreads1 = Math.ceil(
    (target.security.level - target.security.min) * 20
  );
  ...
  while (growThreads > 0 || weakThreads1 > 0 || weakThreads2 > 0) {
    ...
    growThreads = numCycleForGrowthCorrected(
      ns.getServer(target.id),
      target.money.max,
      target.money.available,
      ns.getPlayer(),
    ...
    if (home.threadCount(1.75) >= growThreads + weakThreads1 + weakThreads2) {
      if (weakThreads1 > 0) {
        ns.exec("trhr/bin.wk.js", "home", weakThreads1, target.id);
      }
      if (growThreads > 0) {
        ns.exec("trhr/bin.gr.js", "home", growThreads, target.id);
      }
    } else {
      if (weakThreads1 > 0 && home.threadCount(1.75) > 0) {
        ns.exec(
          "trhr/bin.wk.js",
          "home",
          Math.min(weakThreads1, home.threadCount(1.75)),
          target.id
        );
        ...
        ns.exec(
          "trhr/bin.gr.js",
          "home",
          Math.min(growThreads, home.threadCount(1.75)),
          target.id
        );
        ...
        ns.exec(
          "trhr/bin.wk.js",
          "home",
          Math.min(weakThreads2, home.threadCount(1.75)),
          target.id
        );
      }
    }
  ...
  while (true) {
    ...
    let hackThreads = Math.floor(
      ns.hackAnalyzeThreads(target.id, target.money.max * hackPercent)
    );
    weakThreads1 = Math.ceil(hackThreads / 25);
    growThreads = numCycleForGrowthCorrected(
      ns.getServer(target.id),
      target.money.max,
      target.money.max * (1 - hackPercent),
      ns.getPlayer(),
      home.cores
    );
    weakThreads2 = Math.ceil(growThreads / 12.5);
    ...
    if (available_ram > required_ram) {
      ...
      ns.print(`not enought ram, want ${required_ram}, have ${available_ram}`);
    }
